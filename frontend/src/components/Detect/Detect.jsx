...
const Detect = () => {
  ...
  useEffect(() => {
    async function loadGestureRecognizer() {
      ...
      const recognizer = await GestureRecognizer.createFromOptions(vision, {
        baseOptions: {
          modelAssetPath:
            `${process.env.REACT_APP_FIREBASE_STORAGE_TRAINED_MODEL_URL}`,
        },
        numHands: 2,
  ...
  return (
    <>
      <div className="flex justify-evenly items-center font-sans">
        {accessToken ? (
          <>
            <div style={{ position: "relative" }}>
              <Webcam
                ...
                className="w-[600px] h-[500px] z-[999]"
              />

              <canvas ref={canvasRef} className="absolute top-0 left-0 w-[600px] h-[500px] z-[999]" />

              <div className="flex flex-col items-center justify-center text-center">
                <button onClick={enableCam}
                className="w-[100px] h-[30px] text-[20px] font-semibold bg-gray-500 text-white rounded-lg">
                  {webcamRunning ? "Stop" : "Start"}
                </button>

                <div className="flex items-center justify-start w-full px-4">
                  <p className="text-[20px] w-1/5">{gestureOutput}</p>
            ...
            <div className="flex flex-col justify-center items-center h-[550px] px-4-container">
              <h2 className="text-[25px] uppercase tracking-widest">Image</h2>

              <div className="flex flex-col justify-center items-center w-full h-[400px]">
                {currentImage ? (
                  <img src={currentImage.url} alt={`img ${currentImage.id}`}  className="w-[400px] h-[300px] mb-[8rem]"/>
                ) : (
        ...
        (
          <div className="flex flex-col justify-center items-center my-8">
             ...
             <h1 className="text-[28px] font-bold tracking-[5px]">Please Login !</h1>
             <img src={DisplayImg} alt="diplay-img" className="w-[600px] h-[400px] object-contain my-4 rounded-lg"/>
             <p>
